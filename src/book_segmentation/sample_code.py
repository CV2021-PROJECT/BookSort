...
def downsample(img, num_downsamples, debug=False):
    ...
    for i in range(num_downsamples):
        proc_img = scipy.ndimage.interpolation.zoom(proc_img, 0.5)
...
def sobel_x_squared(img, debug=False):
    ...
    proc_img = cv2.Sobel(img, cv2.CV_64F, 1, 0) ** 2
...
def sobel_y_squared(img, debug=False):
    ...
    proc_img = cv2.Sobel(img, cv2.CV_64F, 0, 1, ksize=-1) ** 2.0
...
def laplace_squared(img, debug=False):
    ...
    proc_img = (sobel_x_img ** 2.0 + sobel_y_img ** 2.0) ** 0.5
...
def digitize(img, num_levels, debug=False):
    ...
    img_max = np.max(img)
    img_min = np.min(img)
    bins = [1.0 * i * (img_max - img_min) / num_levels for i in range(0, num_levels)]
...
def horizontal_dilate_subtract(img, structure_length, iterations, debug=False):
    ...
    proc_img = img - scipy.ndimage.morphology.binary_dilation(
...
def get_book_lines(img, debug=False):
    ...
    # Rotate
    # proc_img = scipy.ndimage.rotate(gs_img, angle=0, reshape=False)
    ...
    final_img, levels = connected_components(final_img, debug=True)

    print("show final image2")
    fig = plt.figure(figsize=(16, 12))
    plt.imshow(final_img, cmap="gray")
    plt.show()

    # Get the lines from the label
    lines = get_lines_from_img(final_img, levels, debug=True)
...
if __name__ == "__main__":
    ...
    ndimage = read_image("sample_inputs/sample3.jpeg")
    lines = get_book_lines(ndimage, debug=True)
    print(lines)
